...
> **한 줄**: 이웃을 **전부** 보지 말고, **매번 고정 개수만 무작위로** 뽑아 쓴다. → 차수가 아무리 커도
> 계산량에 **상한** 이 생긴다.

- **문제 — 이웃 폭발**: [GCN](gcn.md)처럼 모든 이웃을 보면 홉이 깊어질수록 폭발한다. 평균 친구 100명이면
  1-홉 100 → **2-홉 100×100 = 10,000** → 3-홉 100만. 한 노드 계산에 그래프 절반이 딸려옴.
- **해결 — 고정 예산 샘플링**: 층별로 뽑을 수 $S_1, S_2$ 를 정한다(예: $S=[25,10]$).
```
                전부(GCN)            샘플링(S=[25,10])
v의 1-hop :     100명         →      랜덤 25명
   2-hop :   100×100=10,000   →      25×10 = 250명   (≈40배 ↓, 차수와 무관하게 고정)
```
- **미니배치로 학습**: ① 타깃 노드 몇 개(예: 512) 선택 → ② 각자 이웃을 샘플해 **작은 서브그래프** 구성 →
  ③ 그 조각 안에서만 집계 계산. **그래프 전체를 메모리에 올릴 필요가 없다.**
- **얻는 것**: 대규모(수억 노드) 확장 · 귀납적(새 노드는 이웃만 샘플하면 즉석 임베딩) · 매번 다른 표본이라
  약한 정규화 효과.
- **트레이드오프**: 무작위라 매 실행이 조금 다르고 일부 이웃을 놓칠 수 있음 → 학습 땐 매번 새로 샘플, 추론
  땐 표본을 늘리거나 전체 이웃을 쓰기도 함.
- 비유: 내 인맥 분위기를 알려고 **친구 1000명 전부 설문(GCN)** 하지 않고 **무작위 25명만 물어보는(샘플링)** 것.

> 참고: [HGT](../heterogeneous/hgt.md)의 **HGSampling** 은 이 아이디어의 **이종 그래프판** — 타입(저자/논문/학회)이
> 한쪽으로 쏠리지 않게 **타입 균형을 맞춰** 샘플한다.
