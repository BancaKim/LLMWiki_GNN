...
> **한 줄 요약**: 학습 결과물은 **노드별 임베딩 표 $Z\in\mathbb{R}^{N\times d}$**(룩업 테이블)이다.
> 따라서 **그 그래프 한 장의 노드 전용** 이며, 평가도 **같은 그래프** 안에서 **라벨만 가렸다 맞히는** 식이다.

- **학습되는 것 = 출력 그 자체**: 파라미터가 곧 임베딩($Z$의 각 행 = 특정 노드 1개). 새 노드의 임베딩을
  계산해 주는 *함수* 가 없어, 새 노드/새 그래프엔 **재학습** 이 필요하다(→ 얕은·전이적).
- **평가 절차(다중 라벨 분류)**:
  1. 그래프 **전체**(모든 노드·엣지)로 node2vec 학습 → 모든 노드 임베딩 확보(라벨 안 씀).
  2. 노드 라벨 중 **일부 비율만 공개**($T_R$=10\%\~90\%)해 분류기(로지스틱 회귀) 학습.
  3. **같은 그래프의 라벨 가린 노드** 를 예측 → **Micro/Macro-F1**.
- ⚠️ **나누는 건 그래프가 아니라 라벨**: 테스트 노드도 임베딩 학습 때 이미 그래프에 있던 노드다.
  ...
  **서로 독립된 단일-그래프 실험 3개**(한 그래프에서 배워 다른 그래프로 전이하는 게 아님).
